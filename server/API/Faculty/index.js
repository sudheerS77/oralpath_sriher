...
//Models
const { FacultyModel } = require("../../database/department/faculty");
const {
  VisitingFacultyModel,
} = require("../../database/department/visitingFaculty");
...
Router.put("/update-faculty", async (req, res) => {
  try {
    ...
    await FacultyModel.findOneAndUpdate({ _id: data._id }, { $set: data });
...
Router.delete("/delete-f/:_id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ data: faculty, message: "Faculty Deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
Router.post("/add-visiting-faculty", async (req, res) => {
  try {
    const isAvailable = await VisitingFacultyModel.findOne(
      req.body.facultyData
    );
    if (isAvailable) {
...
Router.delete("/delete-vf/:_id", async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        data: faculty,
        message: "Visiting Faculty Deleted successfully",
      });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
