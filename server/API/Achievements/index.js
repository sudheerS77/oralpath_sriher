...
//Models
const { AchievememtModel } = require("../../database/Achievements/index");
...
Router.post("/add-achievements", async (req, res) => {
  try {
    const isAvailable = await AchievememtModel.findOne(
      req.body.achievememtData
    );
    if (isAvailable) {
...
Router.put("/update-achievement", async (req, res) => {
  try {
    ...
    console.log(data);
    await AchievememtModel.findOneAndUpdate({ _id: data._id }, { $set: data });
