...
//Models
const { FeedbackModel } = require("../../database/Feedback/index");
const {
  FacultyFeedbackModel,
} = require("../../database/Feedback/facultyFeedback");
...
Router.get("/faculty-feedback/:_id", async (req, res) => {
  try {
    ...
    var feedbackusers = await FeedbackModel.find({
      faculty_id: req.params._id,
    });
    fdata.map((data) => {
      console.log(data);
    ...
    const rating = [excellent, good, moderate, unsatisified, poor];
    console.log(rating);
