...
//Models
const { ProjectModel } = require("../../database/projects/index");
...
Router.put("/update-project", async (req, res) => {
  try {
    ...
    await ProjectModel.findOneAndUpdate({ _id: data._id }, { $set: data });
...
Router.delete("/delete/:_id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ data: data, message: "Project Deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
